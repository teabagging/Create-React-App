...
The  attribute is used to set the title of the document. It can be defined as a simple [string](#string) or an optional [template object](#template-object).
...
`title.default` can be used to provide a **fallback title** to child route segments that don't define a .
...
>   - `title.template` defined in `layout.js` will not apply to a  defined in a `page.js` of the same route segment.
...
>
> - `title.template` has **no effect** if a route has not defined a  or `title.default`.
...
> - `layout.js`
>
>   -  (string) and `title.default` define the default title for child segments (that do not define their own ). It will augment `title.template` from the closest parent segment if it exists.
...
>   -  (string) defines the routes title. It will augment `title.template` from the closest parent segment if it exists.
